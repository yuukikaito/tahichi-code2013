 ...
 */
const static unsigned char bd_addr[7] = { 0x00, 0x16, 0x53, 0x13, 0xE7, 0xB8,
		0x00 };
...
static float bf_hensa;
static float bf_bf_hensa;
static float bf_cmd_turn;
/*
 * 状態セット用のプライベート関数
 */
...
void RA_linetrace_P(int forward_speed) {
	...
	cmd_turn = Kp * hensa;
	//cmd_turn = Kp * (hensa - bf_hensa) + Ki * hensa + Kd * (hensa - bf_hensa);
...
void RA_linetrace_P_after(int forward_speed) {
	...
	float hensa;
	//static const float i_hensa;
	//static const float d_hensa
	float Kp = 0.5;
	//float Kp = 1.7;
	float cmd_turn = 0;
